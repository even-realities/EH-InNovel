...
actual suspend fun launchSystemCapability(capability: SystemCapability): SystemCapabilityResult {
    val result = launchSystemCapabilityImpl(capability.name).awaitWasm()
    ...
    async (capabilityName) => {
      ...
      try {
        if (typeof navigator === "undefined") {
          return unsupported("navigator 不可用");
        }
        switch (capabilityName) {
          case "TakePhoto": {
            if (typeof document !== "undefined") {
              return await pickFile({ accept: "image/*", capture: "environment" });
            }
            return unsupported("当前环境不支持系统相机拉起");
          }
          case "PickImage": {
            if (typeof document === "undefined") {
              return unsupported("当前环境不支持文件选择器");
            }
            return await pickFile({ accept: "image/*" });
          }
          case "RecordVideo": {
            if (typeof document === "undefined") {
              return unsupported("当前环境不支持系统录像拉起");
            }
            return await pickFile({ accept: "video/*", capture: "environment" });
          }
          case "RecordAudio": {
            if (typeof document === "undefined") {
              return unsupported("当前环境不支持系统录音拉起");
            }
            return await pickFile({ accept: "audio/*", capture: "user" });
          }
          case "PickFile": {
            if (typeof document === "undefined") {
              return unsupported("当前环境不支持文件选择器");
            }
            return await pickFile({ accept: "*/*" });
          }
          case "GetLocation": {
            if (!navigator.geolocation) {
              return unsupported("当前环境不支持 geolocation");
            }
            if (!isSecure()) {
              return unsupported("定位需要 HTTPS 或 localhost 环境");
            }
          ...
          default:
            return unsupported("未识别的能力类型: " + capabilityName);
        }
      } catch (error) {
        ...
        return fail(message || "未知错误");
      }
    }
    """
)
private external fun launchSystemCapabilityImpl(capabilityName: String): Promise<JsAny?>
